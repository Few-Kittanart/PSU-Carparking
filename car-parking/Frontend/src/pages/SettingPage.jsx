...
export default function SettingPage() {
  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
        <CircularProgress />
        <Typography variant="h6" sx={{ ml: 2 }}>กำลังโหลดข้อมูล...</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ p: { xs: 3, md: 5 }, bgcolor: '#f4f6f8', minHeight: '100vh' }}>
      <Typography variant="h4" component="h1" sx={{ fontWeight: 'bold', color: '#ea7f33', mb: 4 }}>
        การตั้งค่าระบบ
      </Typography>
      ...
      <Paper elevation={4} sx={{ p: { xs: 3, md: 5 }, borderRadius: 2, mb: 4 }}>
        <Typography variant="h5" sx={{ mb: 2, fontWeight: 'bold', color: '#424242' }}>
          ข้อมูลทั่วไปของระบบ
        </Typography>
      ...
      <Paper elevation={4} sx={{ p: { xs: 3, md: 5 }, borderRadius: 2, mb: 4 }}>
        ...
        {/* Bank 1 */}
        <Accordion expanded={settings.bank1.show} onChange={() => handleSwitchChange({ target: { checked: !settings.bank1.show } }, "bank1", "show")}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ bgcolor: settings.bank1.show ? '#e8f5e9' : 'transparent' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
              <AccountBalanceIcon sx={{ color: '#2e7d32', mr: 2 }} />
              <Typography variant="subtitle1" sx={{ flexGrow: 1 }}>บัญชีธนาคาร 1</Typography>
              <Switch checked={settings.bank1.show} onChange={(e) => handleSwitchChange(e, "bank1", "show")} onClick={(e) => e.stopPropagation()} />
            </Box>
          </AccordionSummary>
          <AccordionDetails>
            <Grid container spacing={3}>
        ...
        {/* Bank 2 */}
        <Accordion expanded={settings.bank2.show} onChange={() => handleSwitchChange({ target: { checked: !settings.bank2.show } }, "bank2", "show")}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ bgcolor: settings.bank2.show ? '#e8f5e9' : 'transparent' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
              <AccountBalanceIcon sx={{ color: '#1565c0', mr: 2 }} />
              <Typography variant="subtitle1" sx={{ flexGrow: 1 }}>บัญชีธนาคาร 2</Typography>
              <Switch checked={settings.bank2.show} onChange={(e) => handleSwitchChange(e, "bank2", "show")} onClick={(e) => e.stopPropagation()} />
            </Box>
          </AccordionSummary>
          <AccordionDetails>
            <Grid container spacing={3}>
        ...
        {/* PromptPay */}
        <Accordion expanded={settings.promptPay.show} onChange={() => handleSwitchChange({ target: { checked: !settings.promptPay.show } }, "promptPay", "show")}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ bgcolor: settings.promptPay.show ? '#fffde7' : 'transparent' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
              <CreditCardIcon sx={{ color: '#ffc107', mr: 2 }} />
              <Typography variant="subtitle1" sx={{ flexGrow: 1 }}>พร้อมเพย์</Typography>
              <Switch checked={settings.promptPay.show} onChange={(e) => handleSwitchChange(e, "promptPay", "show")} onClick={(e) => e.stopPropagation()} />
            </Box>
          </AccordionSummary>
          <AccordionDetails>
            <Grid container spacing={3}>
        ...
        {/* Credit Card */}
        <Accordion expanded={settings.creditCard.show} onChange={() => handleSwitchChange({ target: { checked: !settings.creditCard.show } }, "creditCard", "show")}>
          <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ bgcolor: settings.creditCard.show ? '#e3f2fd' : 'transparent' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
              <CreditCardIcon sx={{ color: '#039be5', mr: 2 }} />
              <Typography variant="subtitle1" sx={{ flexGrow: 1 }}>บัตรเครดิต</Typography>
              <Switch checked={settings.creditCard.show} onChange={(e) => handleSwitchChange(e, "creditCard", "show")} onClick={(e) => e.stopPropagation()} />
            </Box>
          </AccordionSummary>
          <AccordionDetails>
            <Grid container spacing={3}>
      ...
      <Paper elevation={4} sx={{ p: { xs: 3, md: 5 }, borderRadius: 2, mb: 4 }}>
        ...
        <Grid container spacing={3} alignItems="center">
          <Grid item xs={12} md={6}>
            <InputLabel sx={{ mb: 1, color: 'text.secondary' }}>โลโก้หลัก</InputLabel>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              >
                อัปโหลดไฟล์
                <VisuallyHiddenInput type="file" onChange={(e) => handleFileChange(e, "main")} accept="image/*" />
              </Button>
              {settings.logo.main && (
                <img src={settings.logo.main} alt="Main Logo" style={{ maxWidth: '100px', maxHeight: '100px', borderRadius: '8px', border: '1px solid #ccc' }} />
              )}
            </Box>
          </Grid>
          <Grid item xs={12} md={6}>
            <InputLabel sx={{ mb: 1, color: 'text.secondary' }}>โลโก้รอง</InputLabel>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Button
                component="label"
                variant="contained"
              ...
              >
                อัปโหลดไฟล์
                <VisuallyHiddenInput type="file" onChange={(e) => handleFileChange(e, "sub")} accept="image/*" />
              </Button>
              {settings.logo.sub && (
                <img src={settings.logo.sub} alt="Sub Logo" style={{ maxWidth: '100px', maxHeight: '100px', borderRadius: '8px', border: '1px solid #ccc' }} />
              )}
            </Box>
          </Grid>
        </Grid>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 5 }}>
        <Button
          ...
          disabled={isSaving}
        >
          {isSaving ? <CircularProgress size={24} sx={{ color: 'white' }} /> : 'บันทึก'}
        </Button>
      </Box>
    </Box>
  );
